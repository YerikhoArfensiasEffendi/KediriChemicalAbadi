...
# Tightly crop to content: y from 14 to 225
tight_crop = orig_img.crop((0, 14, 1024, 225))
...
print(f"Upscaling tight crop from {w_c}x{h_c} to {target_w}x{target_h}...")

hd_img = tight_crop.resize((target_w, target_h), Image.Resampling.LANCZOS)

# Enhance clarity and glint
r, g, b, a = hd_img.split()
rgb = Image.merge('RGB', (r, g, b))

sharp = ImageEnhance.Sharpness(rgb).enhance(1.4)
contrast = ImageEnhance.Contrast(sharp).enhance(1.08)
color = ImageEnhance.Color(contrast).enhance(1.06)

r_f, g_f, b_f = color.split()
final_hd = Image.merge('RGBA', (r_f, g_f, b_f, a))

final_hd.save('public/images/navbar_liquid_aligned_4k.png', format='PNG', optimize=True)
print(f"Saved aligned 4K liquid wave to public/images/navbar_liquid_aligned_4k.png! Size: {final_hd.size}")
